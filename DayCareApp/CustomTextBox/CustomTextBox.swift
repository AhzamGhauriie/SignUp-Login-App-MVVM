...
class CustomTextBox: UIView, NibFileOwnerLoadable {
    ...
    private func setFieldType() {
        ...
        case .password, .normal_password,.empty_password:
            self.showPass.isHidden = false
            self.textField.isSecureTextEntry = true
            textField.autocapitalizationType = .none
        case .phone:
            self.textField.keyboardType = .phonePad
        case .general, .none, .name, .nickName, .inviteCode:
    ...
    func setFont(placeholder: String = "",
                 placeHolderColor: UIColor = Color.TextPlaceholder,
                 textColor: UIColor = .white){
        
        let font = Theme.getRegularFont(withSize: 15)
...
class TextBoxViewModel {
    ...
    func validate() -> Bool{
        ...
        guard textFieldText != "" else {
            ...
            case .name:
                errorMsgText = "Full name is required!"
            case .nickName:
                errorMsgText = "Nick name is required!"
            case .email:
            ...
            case .normal_password:
                errorMsgText = "Field is required!"
            case .phone:
                errorMsgText = "Phone number is required!"
            case .inviteCode:
                errorMsgText = "Invite code is required!"
            }
        ...
        case .name:
            if (textFieldText.count < 3)  {
                errorMsgText = "FullName must contain atleast 3 characters"
                return false
            } else if textFieldText.contains("."){
                errorMsgText = "only characters allowed"
                return false
            }
            
        case .nickName:
            if (textFieldText.count < 3)  {
                errorMsgText = "NickName must contain atleast 3 characters"
                return false
            }
            /*           let componentSeperatedBySpaces = textFieldText.components(separatedBy: " ")
             if (componentSeperatedBySpaces.count == 1) || (componentSeperatedBySpaces.count > 1 && componentSeperatedBySpaces[1].count == 0) {
